...
function errorResponse(req, res, type, error) {
    let responseJson = {};
    const status = ~~(type/100);  // Gets only the integer part
    responseJson.type = getErrorUrl(req, type);
    responseJson.status = status;
    responseJson.title = errors[type];
    if (!responseJson.title && config.ENV === 'dev') {
        throw new Error('Title not defined for error with type:' + type);
    }
...
function logTransaction(req, res, duration) {
    ...
    const responseJson = res.locals.responseJson;
    ...
    const logMessage = mergeResponse(req, responseJson, message);
